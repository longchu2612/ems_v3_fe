...
export class ChefComponent implements OnInit {
    constructor(
      ...
      activeButton: string; // Biến để theo dõi nút đang được active

      toggleActive(button: string) {
          this.activeButton = button;
          console.log( this.activeButton);

          if(this.activeButton === 'priorityButton'){
            const data = {
            ...
            this.eventService.setLoading(true)
            this.apiService.getListpost('orders/list-product-by-order', data
            ).subscribe({
              next: res => {
          ...
          } else if (this.activeButton === 'tableButton'){
            const data = {
              "pageIndex": this.pageIndex,
              "pageSize": this.pageSize,
            }
