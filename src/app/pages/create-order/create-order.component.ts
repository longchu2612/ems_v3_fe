...
export class CreateOrderComponent implements OnInit {
    ...
    viewDetails(data: any) {
        this.router.navigate(['/details-transactions', data.transaction_id]);
    }
    onImgError(event) {
    ...
    detailQRCode() {
        this.router.navigate(['/detail-create-order'])
    }
    ...
    makeForm(d?) {
        ...
        return this.formBuilder.group({
            ...
            mid: [data.mid, [Validators.required]],
    ...
    qrCodeValue: string;// khai báo 1 biến mới
    // gán biến string vào object item => this.qrCodeValue = res['data'] ? res['data'] : '' 
    //res['data] => trỏ thẳng vào biến data trong privew
    ...
    saveData() {
        ...
        if (this.formData.valid) {
            ...
            if (value.id) {
                ...
                // this.apiService.addQRCode(this.addQR.amount, this.addQR.bankaccount_id, this.addQR.mid ).subscribe({
                //     next: res => {
                //         this.qrCodeValue = res['data'] ? res['data'] : ''
                //         // anh nhận được cái qrstring ở đây đúng không anh ?
                //         // thì mình call trong này luôn để đỡ phải async await
                //         this.apiService.displayQRCode(this.addQR.amount, this.addQR.bankaccount_id, this.addQR.mid, this.qrCodeValue).subscribe(res => {
                //             this.formData.reset()// reset
                //             // this.fetchData()
                //             // this.apiService.showToast('', 'Thêm mới thành công', 'success')
                //             this.eventService.setLoading(false)
                //             // this.modalService.dismissAll() : đóng tất cả
                //             console.log(res)
                //         })
