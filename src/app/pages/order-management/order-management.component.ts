...
fetchData() {
    const data = {
        ...
        "status" : 4,
        "fromDate" :this.filter.start_date ,
        "toDate" : this.filter.end_date ,
        "cashierName": this.filter.cashierName
    }

    this.eventService.setLoading(true)
    this.apiService.getListpost('orders/list', data
    ).subscribe({
        next: res => {
            console.log(res)
...
viewDetails(data: any) {
    this.router.navigate(['ordersmanagement/order-details', data.id]);
}
onImgError(event) {
...
saveData() {
    ...
    if (this.formData.valid) {
        ...
        this.eventService.setLoading(true)
        if (value.id) {
            this.apiService.editItem('orders/update', value.id, value).subscribe({
                next: res => {
                    this.fetchData()
            ...
            this.apiService.addItem('orders/add', value).subscribe({
                next: res => {
                    this.fetchData()
